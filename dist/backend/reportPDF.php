...
$penalty = json_decode($_POST['penalty'], true);
...
$tcpdf->Ln(11);
$tcpdf->SetFont('dejavusans', 'B', 18);
$tcpdf->Cell(0, 0, 'Проверка на наличие штрафов', 0, 0, 'C');
$tcpdf->Ln(11);
if($penalty != null) {
    foreach($penalty as $tx) {
        $tcpdf->Ln(6);
        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Скидка: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["Discount"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Скидка до: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["DateDiscount"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Расшифровка КоАП: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->writeHTMLCell(120, 0, '', '',  $tx["KoAPtext"], 0, 1, 0, true, 'L', true);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Код КоАП: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["KoAPcode"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Номер постановления: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["NumPost"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'КБК: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["kbk"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Сумма штрафа: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["Summa"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Дата постановления: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["DatePost"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Подразделение: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->writeHTMLCell(120, 0, '', '',  $tx["division_name"], 0, 1, 0, true, 'L', true);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Адрес: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["division_address"]);
        $tcpdf->Ln(8);

        $tcpdf->SetFont('dejavusans', '', 10);
        $tcpdf->Cell(57, 0, 'Координаты: ');
        $tcpdf->SetFont('freesans', 'B', 10);
        $tcpdf->Cell(57, 0, $tx["division_coords"]);
        $tcpdf->Ln(8);
    ...
    $tcpdf->Ln(11);
    $tcpdf->SetFont('dejavusans', '', 10);
    $tcpdf->Cell(0, 0, 'Штрафы не найдены', 0, 0, 'C');
    $tcpdf->Ln(10);
    $tcpdf->Image($ok, '', '', 20, 17, 'PNG', false, 'C', false, 300, 'C', false, false, 0, false, false, false);
    $tcpdf->Ln(24);
}
